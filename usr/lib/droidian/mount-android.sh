#!/bin/bash
# Droidian Android partition mount script for Xiaomi Pad 6 (pipa)
# Mounts vendor/odm/product from /userdata/ image files.
...
# Mount vendor_dlkm if available
...
# Bind-mount /vendor if we should
...
fstab=$(ls /vendor/etc/fstab* 2>/dev/null)
[ -z "$fstab" ] && echo "fstab not found, skipping" && fstab=/dev/null
...
# Mount odm/product from img files if available
for part in odm product; do
    imgfile="/userdata/${part}.img"
    mntpoint="/${part}"
    if [ -f "$imgfile" ] && ! mountpoint -q -- "$mntpoint"; then
        echo "Mounting $imgfile as $mntpoint"
        mkdir -p "$mntpoint"
        mount -o ro,loop "$imgfile" "$mntpoint"
        if [ -n "${BIND_MOUNT_PATH}" ] && [[ "$mntpoint" != /mnt/* ]]; then
            mkdir -p "${BIND_MOUNT_PATH}/${part}"
            mount -o bind "$mntpoint" "${BIND_MOUNT_PATH}/${part}"
        fi
    fi
done
